...
function Employees() {
  ...
  const [form, setForm] = useState({ name: "", role: "", site: "", siteCustom: "", contactNo: "", salary: "" });
  ...
  useEffect(() => {
    ...
    if (user?.role === "manager") setForm((prev) => ({ ...prev, site: user.site }));
  }, [user]);
  ...
  const fetchEmployees = async () => {
    try {
      ...
      setEmployees(user.role === "manager" ? data.filter((e) => e.site === user.site) : data);
    } catch (err) { console.error(err); setEmployees([]); }
  };

  useEffect(() => { fetchEmployees(); }, []);

  const resetForm = () => { setForm({ name: "", role: "", site: user.role === "manager" ? user.site : "", siteCustom: "", contactNo: "", salary: "" }); setEditingId(null); setAadhaarFile(null); setPanFile(null); };

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!/^\d{10}$/.test(form.contactNo)) { alert("Enter valid 10 digit number"); return; }
    const finalSite = form.site === "Other" ? form.siteCustom.trim() : form.site;
    if (!finalSite) { alert("Site required"); return; }
    ...
    try {
      if (editingId) await axios.put(`https://attendance-management-backend-vh2w.onrender.com/api/employees/${editingId}`, data);
      else await axios.post("https://attendance-management-backend-vh2w.onrender.com/api/employees", data);
  ...
  const deleteEmployee = async (id) => { if (!window.confirm("Delete employee?")) return; try { await axios.delete(`https://attendance-management-backend-vh2w.onrender.com/api/employees/${id}`); fetchEmployees(); } catch (err) { console.error(err); } };
        ...
        {user.role === "admin" ? (
          <>
            <select value={form.site} onChange={(e) => setForm({ ...form, site: e.target.value })} required>
              ...
              {defaultSites.map((s) => (<option key={s} value={s}>{s}</option>))}
            </select>
            {form.site === "Other" && (<input placeholder="Custom Site Name" value={form.siteCustom} onChange={(e) => setForm({ ...form, siteCustom: e.target.value })} required />)}
          </>
        ) : (<input value={user.site} readOnly />)}

        <input placeholder="Contact Number" value={form.contactNo} onChange={(e) => setForm({ ...form, contactNo: e.target.value })} maxLength={10} required />
        <input placeholder="Salary" type="number" value={form.salary} onChange={(e) => setForm({ ...form, salary: e.target.value })} required />

        <label>Aadhaar Card: <input type="file" accept="image/*" onChange={(e) => setAadhaarFile(e.target.files[0])} /></label>
        <label>PAN Card: <input type="file" accept="image/*" onChange={(e) => setPanFile(e.target.files[0])} /></label>
      ...
      {allSites.map((site) => (
        <div key={site}>
          ...
          <table border="1" width="100%">
            <thead>
              <tr>
                <th>#</th><th>Name</th><th>Role</th><th>Contact</th><th>Salary</th><th>Aadhaar</th><th>PAN</th><th>Action</th>
              </tr>
            </thead>
            <tbody>
              {employees.filter((e) => e.site === site).map((emp, i) => (
                <tr key={emp._id}>
